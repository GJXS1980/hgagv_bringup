...
StatusPublisher::StatusPublisher()
{
   ...
   power_info = mNH.advertise<std_msgs::Float32MultiArray>("/Power_And_Distance", 10);
   ir_sensor = mNH.advertise<std_msgs::UInt32MultiArray>("/Ir_Sensor_Data", 10);
   ult_sensor = mNH.advertise<std_msgs::Float32MultiArray>("/Ult_Sensor_Data", 10);
    imu_sensor = mNH.advertise<std_msgs::Float64>("/IMU_Sensor_Data", 10);
    stop_sensor = mNH.advertise<std_msgs::Int32>("/Stop_Sensor_Data", 10);
...
void StatusPublisher::Update_imu(const char data[], unsigned int len)
{
    ...
    std_msgs::Float32MultiArray Power_Distance; 
    std_msgs::UInt32MultiArray Ir_Sensor; 
    std_msgs::Float32MultiArray Ult_Sensor; 
    std_msgs::Float64 IMU_Sensor; 
    std_msgs::Int32 Stop_Sensor; 
    ...
    for(i=0;i<len;i++)
    {
      //ROS_ERROR("current2 %x",data[i]);
        ...
        else
        {
		   ...
		   std::cout<<"-----------------------------------------------------"<<std::endl;
		   for(int i=0; i<4;i++)
                   	std::cout<<"sonar_distance["<<i<<"]: "<<car_status.sonar_distance[i]<<std::endl;

                   std::cout<<"status:"<<car_status.status<<std::endl;
                   std::cout<<"power:"<<car_status.power<<std::endl;
                   std::cout<<"theta_imu:"<<car_status.theta_imu<<std::endl;
                   std::cout<<"encoder_ppr_imu:"<<car_status.encoder_ppr_imu<<std::endl;
                   std::cout<<"encoder_delta_r:"<<car_status.encoder_delta_r<<std::endl;
                   std::cout<<"encoder_delta_l:"<<car_status.encoder_delta_l<<std::endl;
                   std::cout<<"upwoard:"<<car_status.upwoard<<std::endl;
                   std::cout<<"hbz_status:"<<car_status.hbz_status<<std::endl;


		   for(int i=0; i<4;i++)
                   	std::cout<<"quat["<<i<<"]: "<<car_status.quat[i]<<std::endl;

		   for(int i=0; i<9;i++)
                   	std::cout<<"IMU["<<i<<"]: "<<car_status.IMU[i]<<std::endl;

                   std::cout<<"theta:"<<car_status.theta<<std::endl;
		   std::cout<<"time_stamp_imu:"<<car_status.time_stamp_imu<<std::endl;
		   std::cout<<"current:"<<car_status.current<<std::endl;
		   std::cout<<"left_sensor2:"<<car_status.left_sensor2<<std::endl;
		   std::cout<<"right_sensor2:"<<car_status.right_sensor2<<std::endl;
		   std::cout<<"distance1:"<<car_status.distance1<<std::endl;
#endif

                  Power_Distance.data.push_back(car_status.power);  			//电压
                  Power_Distance.data.push_back(car_status.current);			//电流
                  Power_Distance.data.push_back(car_status.distance1);			//超声波
                  
                  Ir_Sensor.data.push_back(car_status.left_sensor2);			//左红外
                  Ir_Sensor.data.push_back(car_status.right_sensor2);
                  Ir_Sensor.data.push_back(car_status.time_stamp_imu);			//时间戳

                  Ult_Sensor.data.push_back(car_status.sonar_distance[0]);     // 左前超声波
                  Ult_Sensor.data.push_back(car_status.sonar_distance[1]);     // 右前超声波
                  IMU_Sensor.data = car_status.theta_imu;                                         //陀螺仪数据
                  Stop_Sensor.data  = car_status.hbz_status ;                                    //防撞条 急停

                  power_info.publish(Power_Distance);  
                  ir_sensor.publish(Ir_Sensor);  
                  ult_sensor.publish(Ult_Sensor); 
                  imu_sensor.publish(IMU_Sensor);    
                  stop_sensor.publish(Stop_Sensor);
...
void StatusPublisher::Refresh()
{
    ...
    static bool yaw_omega_ready = false;
    {
      ...
      if(mbUpdated_imu)
      {
        ...
        {
          CarIMU.angular_velocity.x=car_status.IMU[4]* PI /180.0f; //陀螺仪安装位置
          CarIMU.angular_velocity.y=car_status.IMU[3]* PI /180.0f;
          CarIMU.angular_velocity.z=car_status.IMU[5]* PI /180.0f;
        ...
        if(distance_sum_i%10==0)
        {
          ...
          distances_[0] = 4.0;//car_status.sonar_distance[0];
          distances_[1] = 4.0;//car_status.sonar_distance[1];
          distances_[2] = 4.0;//car_status.sonar_distance[2];
          distances_[3] = 4.0;//car_status.sonar_distance[3];
